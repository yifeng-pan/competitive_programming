...
// used in dp array, to same on memory
// by no means the best pratice
struct simple_knapsack{
    int value;
...
knapsack maximize(knapsack k){
    ...
    simple_knapsack base_case {0};
    for(int n = 0; n < N; ++n)
        base_case.choices.push_back(false);

    // row is from 0 elements to n elements hence n + 1, not n
    // col simular reason
    simple_knapsack K[N + 1][C + 1];
    ...
    for(int n = 0; n <= N; ++n){
        for(int c = 0; c <= C; ++c){
            if(n <= 0 || c <= 0)
                K[n][c] = base_case; // k is default
            else if(k.items[n - 1].weight > c)
                K[n][c] = K[n - 1][c];
            else{
                simple_knapsack take {K[n - 1][c - k.items[n - 1].weight]};
                take.value += k.items[n - 1].value;
                take.choices[n - 1] = true;

                simple_knapsack not_take {K[n - 1][c]};
    ...
    k.value = K[N][C].value;
    for(int n = 0; n < N; ++n)
        k.items[n].choose = K[N][C].choices[n];
